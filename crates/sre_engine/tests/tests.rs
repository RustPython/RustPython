...
mod tests {
    ...
    #[test]
    fn possessive_repeat_fullmatch() {
    ...
    #[test]
    fn search_literal_prefix_longer_than_skip() {
        // The INFO block carries prefix_len=4 and prefix_skip=2, so the tail
        // match has to resume at the skip boundary rather than past the whole
        // prefix.
        // pattern p = re.compile('ab(cd)')
        // START GENERATED by generate_tests.py
        #[rustfmt::skip] let p = Pattern { pattern: "ab(cd)", code: &[14, 14, 1, 4, 4, 4, 2, 97, 98, 99, 100, 0, 0, 0, 0, 16, 97, 16, 98, 17, 0, 16, 99, 16, 100, 17, 1, 1] };
        // END GENERATED
        let (req, mut state) = p.state("xabcdcd");
        assert!(state.search(req));
        assert_eq!(state.start, 1);
        assert_eq!(state.cursor.position, 5);
    }
