...
impl CodeInfo {
    pub fn finalize_code(
        ...
        self.deoptimize_borrow_after_assertion_success_tail();
        self.deoptimize_borrow_in_unprotected_debug_four_tails();
            ...
            block.instructions.retain(|ins| {
                let keep = 'keep: {
                    if matches!(ins.instr.real(), Some(Instruction::Nop)) {
                        if ins.remove_no_location_nop && instruction_lineno(ins) < 0 {
                            break 'keep false;
                        }
    ...
    fn deoptimize_borrow_after_assertion_success_tail(&mut self) {
    ...
    fn deoptimize_borrow_in_unprotected_debug_four_tails(&mut self) {
        fn deoptimize_block_borrows(block: &mut Block) {
            for info in &mut block.instructions {
                match info.instr.real() {
                    Some(Instruction::LoadFastBorrow { .. }) => {
    ...
    fn deoptimize_borrow_in_async_finally_early_return_tail(&mut self) {
        ...
        let send_pos = self
            .blocks
            .iter()
            .position(|block| !block.cold && !block_is_exceptional(block) && block_has_send(block));
        let Some(send_pos) = send_pos else {
            return;
        };
        let has_early_return = self.blocks[..send_pos]
            .iter()
            .any(|block| !block.cold && !block_is_exceptional(block) && block_has_return(block));
        if !has_early_return {
            return;
        }
    ...
    fn deoptimize_borrow_after_protected_store_tail(&mut self) {
        ...
        for block in &self.blocks {
            ...
            if segment.is_empty()
                ...
                || predecessors[segment[0].0.idx()].iter().any(|pred| {
                    contains_debug_four_guard(&self.blocks[pred.idx()], &self.metadata.names)
                })
...
impl CodeInfo {
    ...
    pub(crate) fn debug_late_cfg_trace(mut self) -> crate::InternalResult<Vec<(String, String)>> {
        ...
        self.deoptimize_borrow_after_assertion_success_tail();
        trace.push((
            "after_deoptimize_borrow_after_assertion_success_tail".to_owned(),
            self.debug_block_dump(),
        ));
        self.deoptimize_borrow_in_unprotected_debug_four_tails();
        trace.push((
            "after_deoptimize_borrow_in_unprotected_debug_four_tails".to_owned(),
            self.debug_block_dump(),
        ));
        self.deoptimize_borrow_for_folded_nonliteral_exprs();
        trace.push((
...
fn remove_redundant_nops_in_blocks(blocks: &mut [Block]) -> usize {
    ...
    let mut fallthrough_prev_pop_top_lineno = vec![None; blocks.len()];
    ...
    for &block_idx in &block_order {
        ...
        if prev_nonempty != BlockIdx::NULL
            && block_has_fallthrough(&blocks[prev_nonempty.idx()])
            && next_nonempty_block(blocks, blocks[prev_nonempty.idx()].next) == block_idx
            && let Some(last) = blocks[prev_nonempty.idx()].instructions.last()
            && matches!(last.instr.real(), Some(Instruction::PopTop))
        {
            fallthrough_prev_pop_top_lineno[block_idx.idx()] = Some(instruction_lineno(last));
        }
...
fn reorder_conditional_body_and_implicit_continue_blocks(blocks: &mut [Block]) {
