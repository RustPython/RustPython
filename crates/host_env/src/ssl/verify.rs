...
//! Certificate loading and rustls verifiers for `_ssl`.
...
use super::cert::is_ca_certificate;
...
impl<'a> CertLoader<'a> {
    ...
    pub fn load_from_dir(&mut self, dir_path: &str) -> Result<CertStats, std::io::Error> {
        let entries = crate::fs::read_dir(dir_path)?;
        ...
        for entry in entries {
            let entry = entry?;
            let path = entry.path();
            if is_capath_hash_name(&entry.file_name()) && crate::fs::is_file(&path) {
                paths.push(path);
            }
        }
        paths.sort();
        ...
        for path in paths {
            let Ok(contents) = crate::fs::read(path) else {
                continue;
            };
...
pub fn load_cert_chain_from_file(
    cert_path: &str,
    key_path: &str,
    password: Option<&str>,
