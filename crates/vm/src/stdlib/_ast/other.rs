...
impl Node for ast::ConversionFlag {
    ...
    fn ast_from_object(
        ...
        match node_object_to_i32(ctx, object)? {
            -1 => Ok(Self::None),
            x if x == b's' as i32 => Ok(Self::Str),
            x if x == b'r' as i32 => Ok(Self::Repr),
            x if x == b'a' as i32 => Ok(Self::Ascii),
            x => Err(ctx.new_system_error(format!("Unrecognized conversion character {x}"))),
        }
    }
}
...
impl Node for ast::Decorator {
    ...
    fn ast_from_object(
        ...
        let expression = ast::Expr::ast_from_object(ctx, source_file, object)?;
...
impl Node for ast::Alias {
    ...
    fn ast_from_object(
        ...
        Ok(Self {
            node_index: Default::default(),
            name: get_required_identifier_field(ctx, source_file, &object, "name", "alias")?,
            asname: get_node_field_opt(ctx, &object, "asname")?
                .map(|obj| Node::ast_from_object(ctx, source_file, obj))
                .transpose()?,
            range: range_from_object(ctx, source_file, object, "alias")?,
        })
    }
}
...
impl Node for ast::WithItem {
    ...
    fn ast_from_object(
        ...
        Ok(Self {
            ...
            optional_vars: get_node_field_opt(ctx, &object, "optional_vars")?
                .map(|obj| Node::ast_from_object(ctx, source_file, obj))
                .transpose()?,
