    ...
    fn deserialize_value_from_file(
        ...
        match marshal::deserialize_value(rdr, PyMarshalBag::new(vm, &pending_error, allow_code)) {
            ...
            Err(error) => Err(pending_error.into_inner().unwrap_or_else(|| match error {
                // r_object: the data ran out where an object should have started
                marshal::MarshalError::EofObject => {
                    vm.new_eof_error("EOF read where object expected")
                }
                // r_byte: a length or flag byte that never came
                marshal::MarshalError::Eof | marshal::MarshalError::EofByte => {
                    vm.new_eof_error("EOF read where not expected")
                }
                // r_string: a payload cut short, worded by where it was read from
                marshal::MarshalError::DataTooShort => vm.new_eof_error(if from_file {
