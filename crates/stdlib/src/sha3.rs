...
mod _sha3 {
    ...
    #[pyfunction]
    fn sha3_224(args: FuncArgs, vm: &VirtualMachine) -> PyResult {
        Ok(local_sha3_224(args, vm)?.into_pyobject(vm))
    }

    #[pyfunction]
    fn sha3_256(args: FuncArgs, vm: &VirtualMachine) -> PyResult {
        Ok(local_sha3_256(args, vm)?.into_pyobject(vm))
    }

    #[pyfunction]
    fn sha3_384(args: FuncArgs, vm: &VirtualMachine) -> PyResult {
        Ok(local_sha3_384(args, vm)?.into_pyobject(vm))
    }

    #[pyfunction]
    fn sha3_512(args: FuncArgs, vm: &VirtualMachine) -> PyResult {
        Ok(local_sha3_512(args, vm)?.into_pyobject(vm))
    }

    #[pyfunction]
    fn shake_128(args: FuncArgs, vm: &VirtualMachine) -> PyResult {
        Ok(local_shake_128(args, vm)?.into_pyobject(vm))
    }

    #[pyfunction]
    fn shake_256(args: FuncArgs, vm: &VirtualMachine) -> PyResult {
