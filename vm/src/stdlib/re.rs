...
pub fn make_module(vm: &VirtualMachine) -> PyObjectRef {
    ...
    py_module!(vm, "regex_crate", {
        "compile" => ctx.new_function(re_compile),
        "escape" => ctx.new_function(re_escape),
        "purge" => ctx.new_function(re_purge),
        "Match" => match_type,
        "match" => ctx.new_function(re_match),
        "Pattern" => pattern_type,
        "search" => ctx.new_function(re_search),
        "sub" => ctx.new_function(re_sub),
        "findall" => ctx.new_function(re_findall),
        "split" => ctx.new_function(re_split),
