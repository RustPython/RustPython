...
use crate::pyobject::{PyValue, StaticType, PyRef, PyObjectRef, PyResult};
...
impl ExternalFunctions {
    pub fn call_function(&self, function: &str, arguments: Vec<CDataObject>, vm: &VirtualMachine) -> PyResult<PyObjectRef> {
    ...
    pub unsafe fn get_or_insert_lib(&mut self, library_path: String) -> Result<&Library,libloading::Error>{
        
        let library = self.libraries.entry(library_path).or_insert({
            Arc::new(Library::new(library_path)?)
        });
    ...
    pub fn get_or_insert_fn(&mut self, func_name: String, library_path: String, vm: &VirtualMachine) -> PyResult<&mut Arc<FunctionProxy>>{
        ...
        match self.libraries.get(&library_path) {
            Some(library) => Ok(self.functions.entry(f_name).or_insert({
                Arc::new(FunctionProxy {_name: f_name, _lib: *library})
            })),
