...
# RustPython: additional cache tags to try when looking up bytecode files.
# RustPython itself writes .pyc with its own cache_tag, but reading .pyc
# generated by CPython (e.g. via `python3 -m compileall`) requires accepting
# the CPython tag too. These act as read-only fallbacks; bytecode writes
# always use sys.implementation.cache_tag.
_RP_FALLBACK_CACHE_TAGS = ('cpython-314',)
...
class SourceLoader(_LoaderBasics):
    ...
    def get_code(self, fullname):
        ...
        try:
            ...
            # RustPython: include CPython-tagged variants as read-only
            # fallbacks so .pyc generated by stock CPython can be reused.
            _rp_primary_tag = sys.implementation.cache_tag
            _rp_candidate_paths = [bytecode_path]
            if _rp_primary_tag:
                _rp_marker = f'.{_rp_primary_tag}.'
                for _rp_alt_tag in _RP_FALLBACK_CACHE_TAGS:
                    if _rp_alt_tag and _rp_alt_tag != _rp_primary_tag:
                        _rp_alt = bytecode_path.replace(
                            _rp_marker, f'.{_rp_alt_tag}.', 1
                        )
                        if _rp_alt != bytecode_path:
                            _rp_candidate_paths.append(_rp_alt)
            ...
            try:
                ...
                # bytecode_path stays as the write target (primary RustPython
                # tag); _rp_read_path tracks where the actual .pyc was found
                # for verbose/error messages.
                data = None
                _rp_read_path = bytecode_path
                for _rp_candidate in _rp_candidate_paths:
                    try:
                        data = self.get_data(_rp_candidate)
                    except OSError:
                        data = None
                        continue
                    _rp_read_path = _rp_candidate
                    break
                if data is not None:
                    exc_details = {
                        'name': fullname,
                    ...
                    try:
                        ...
                        _bootstrap._verbose_message('{} matches {}', _rp_read_path,
                                                    source_path)
                        return _compile_bytecode(bytes_data, name=fullname,
                                                 bytecode_path=_rp_read_path,
