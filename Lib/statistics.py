...
def kde(data, h, kernel='normal', *, cumulative=False):
    ...
    if kernel == "normal" or kernel == "gauss":
        sqrt2pi = sqrt(2 * pi)
        sqrt2 = sqrt(2)
        ...
        support = None
    elif kernel == "rectangular" or kernel == "uniform":
        K = lambda t: 1/2
        ...
        support = 1.0
    elif kernel == "parabolic" or kernel == "epanechnikov":
        K = lambda t: 3/4 * (1.0 - t * t)
        W = lambda t: -1/4 * t**3 + 3/4 * t + 1/2
        support = 1.
    elif kernel == "quartic" or kernel == "biweight":
