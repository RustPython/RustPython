...
def state_has_meaningful_duration(state):
...
class TestResult:
    ...
    def is_failed(self, fail_env_changed: bool) -> bool:
        if self.state == "ENV_CHANGED":
            return fail_env_changed
        return state_is_failed(self.state)
    ...
    def has_meaningful_duration(self):
        return state_has_meaningful_duration(self.state)

    def set_env_changed(self):
        if self.state is None or self.state == "PASSED":
            self.state = "ENV_CHANGED"

    def must_stop(self, fail_fast: bool, fail_env_changed: bool) -> bool:
        if state_must_stop(self.state):
            return True
